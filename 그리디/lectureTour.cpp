...
최소 일부터 시작해서 강연비를 저장한다.
만약 탐색중인 강연의 비용이 해당 날짜에 저장되어 있는 비용보다 높으면 교체하고 기존 비용을 따로 추출한다.
추출한 비용을 이전까지의 날짜중 최소 비용을 가진 날짜와 비교해서 더 높은 값으로 교체하는 방식으로 진행한다.
이전까지의 날짜중 최소값을 현재 값으로 교체하면 최대 비용이 된다고 방향을 정해놓고 코딩하는 그리디 문제다.
*/
