...
같은 양의 물품을 탐색했고, 같은 무게를 채웠다면 동일한 탐색을 수행한다는 점을 이용해 메모이제이션 한다.
다음 물품을 넣는 경우와 넣지 않는 경우에서 가치를 가져와 비교하여 더 높은 것을 현재 물품 위치에 기록한다.

for문을 이용한 DP에서도 마찬가지지만 방향만 바뀐다.
현재 물품까지의 최적 가치는 현재 물품을 넣는 경우와 넣지 않는 경우를 비교해 더 높은 것을 기록한다.
'''
