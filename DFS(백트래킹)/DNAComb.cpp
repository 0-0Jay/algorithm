...
// arr배열에 입력으로 주어지는 모든 DNA 조각을 저장했다. check배열은 중복된 조각을 탐색하지 않도록 하기위한 장치로 사용했다.
// #1 함수에 현재 조합된 DNA(s), 탐색한 조각 수(d), 이전에 탐색한 DNA 조각(bef)를 인수로 주었다.
//    이 때, bef가 필요한 이유는 문제의 조건인 '인접한 문자열의 공통부분만 제거할 수 있다.'를 만족하기 위한 재료이다.
...
// #2 d가 n과 같다는 것은 전체 조각을 탐색했다는 의미이므로 현재 조합된 DNA배열의 길이를 m과 비교해, 최소값만 저장하게 했다.
// #3 check배열은 arr배열의 인덱스를 공유시켰다. 즉 arr[i]를 탐색했다면, check[i]에 1이 저장되고, check[i]가 1인지 확인하는 것으로 중복 탐색을 피하게 했다.
// #4 tmp는 현재 탐색중인 문자열의 길이를 저장했다.
// #5 #1에서 설명한 바와 같이, bef 오른쪽 끝부터, r(현재 탐색중인 조각)의 왼쪽 끝부터 시작하여 substr을 이용해 같은 길이의 문자열을 잘라서 비교했다.
