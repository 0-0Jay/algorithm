...
이를 해결하기 위해 덱 자료구조를 사용한다.
덱의 0번 인덱스는 현재 범위에서의 최소값, 나머지 인덱스는 현재 범위에서 다음 최소값 후보를 둔다.
최소값 후보를 두는 이유는, 0번 인덱스의 최소값이 범위를 벗어났을 때, 바로 다음 최소값이 될 수 있기 때문이다.

매 숫자마다 다음 과정을 반복한다.
1. 덱에 숫자가 있을 경우, 덱의 0번 인덱스가 범위 안에 속해 있는지 확인하고, 아니라면 덱에서 제거한다.
2. 덱의 숫자들이 현재 숫자보다 작으면, 범위 안의 최소값이 갱신되어야 하므로 교체한다.
-> 덱에 포함된 숫자는 1번 과정에 의해 반드시 범위 안의 값만 존재한다.
-> 예를 들어, 덱에 [3, 5]가 있고 새로 2가 들어오는 경우 3, 5의 유효 범위에 관계 없이 모두 제거된다.
3. 현재 값 arr[i]를 덱에 추가한다.
4. 덱의 왼쪽 끝을 출력한다.
-> 2번 과정에 의해 덱의 왼쪽 끝 즉, 0번 인덱스는 반드시 범위 내 최소값이 된다.
'''
